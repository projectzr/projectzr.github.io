<?php include 'includes/estrutura/assinatura.php'; ?>

<!DOCTYPE html>
<html lang="ca">

<head>

    <!-- Informações básicas
    –––––––––––––––––––––––––––––––––––––––––––––––––– -->
    <meta charset="utf-8">
    <title>Brasil Literário - Realismo: Principais Obras</title>
    <?php include 'includes/estrutura/head.php'; ?>
    <link rel="stylesheet" href="css/realismo.css">
</head>

<body>

    <!-- Menu de Navegação
    –––––––––––––––––––––––––––––––––––––––––––––––––– -->
    <?php include 'includes/estrutura/navbar.php'; ?>

    <!-- Header
    –––––––––––––––––––––––––––––––––––––––––––––––––– -->
    <?php $titulo='Realismo'; $subtitulo='Principais Obras'; include 'includes/section-header.php'; ?>
    <?php
    $nomea="Contexto Histórico";
    $linka="realismo-contexto-historico.php";
    $nomeb="Autor de destaque";
    $linkb="realismo-autor-destaque.php";
    include 'includes/breadcrumb.php'; 
    ?>
        <!-- Conteúdo
    –––––––––––––––––––––––––––––––––––––––––––––––––– -->
        <main>
            <h1 style="display: none">Conteúdo</h1>

            <!-- Escolas Literárias
        –––––––––––––––––––––––––––––––––––––––––––––––––– -->
            <div class="container section">
                <div class="row text-left">
                    <h1 class="parameio titulo centro">Realismo no Brasil </h1>
                    <div class="realismodivisor"></div>
                    <article class="col-xs-12 col-md-6 col-lg-6">
                        <h2 style="display: none">Realismo no Brasil e suas principais obras</h2>
                        <p class="pararealismo capitular">Aliquam erat volutpat. Phasellus semper cursus suscipit. Curabitur eget lacus finibus, porta metus id, tincidunt velit. Morbi ac augue ac libero rhoncus rhoncus. Nullam mattis semper magna, non vulputate lorem. Fusce quis tortor semper, congue est sit amet, ornare nibh. Fusce et quam quis urna aliquam finibus. Morbi nibh purus, mattis nec ligula eget, fermentum venenatis elit. Maecenas dictum massa ut aliquet sodales. Sed ut fermentum leo. Nulla lobortis, justo vestibulum gravida fringilla, ligula odio ullamcorper ligula, ut aliquam elit libero eget nulla. Quisque vitae velit accumsan, bibendum ante vel, tempor augue. Proin eget erat quis nibh malesuada suscipit quis ac est.</p>
                        <p class="pararealismo">Nulla metus magna, suscipit sed scelerisque quis, pretium vitae turpis. Mauris consectetur consequat nisi vel pretium. Praesent tristique leo sed elit condimentum, id vulputate leo elementum. Aliquam consectetur sapien posuere ex finibus fringilla. Quisque faucibus, diam sit amet lacinia pharetra, nibh lacus blandit lacus, sagittis porta magna diam vel ligula. Suspendisse semper suscipit finibus. Maecenas hendrerit est tortor, a volutpat odio ornare id. Duis sit amet quam volutpat, consequat ante non, dapibus erat. Integer aliquet, eros id facilisis lobortis, orci tortor dictum ligula, in finibus felis leo in neque. Suspendisse non sem sapien. Sed non lectus at ipsum ornare convallis at vitae mauris. Donec gravida neque vitae mollis ultrices. Proin a velit non est sollicitudin faucibus.</p>
                        <p class="pararealismo">Morbi convallis turpis porttitor, gravida libero ac, nulla efficitur imperdiet tortor, ac dapibus libero dictum non. Donec id tortor et mauris condimentum sodales. In varius posuere nunc, eget tincidunt mauris blandit a. Nullam ut nunc nulla.</p>
                    </article>
                    <aside class="col-xs-12 col-md-6 col-lg-6">
                        <p class="pararealismo">Nunc sodales ornare eros. Praesent feugiat semper sollicitudin. Cras consequat lorem urna, id placerat risus feugiat at.Morbi egestas lorem leo, vitae feugiat nisl aliquam nec. Morbi dictum nisl neque. Nunc sodales ornare eros. Fusce vel libero non eros iaculis ultrices nec vitae nulla. Aliquam sollicitudin sem ac mi molestie, id pulvinar dui facilisis. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
                        <p class="pararealismo">Duis vel vehicula felis. Sed mattis non massa non ornare. Curabitur sed accumsan dui. Mauris commodo tortor id tellus vehicula tempus. Nunc ornare volutpat dui at volutpat.Nam a magna finibus, gravida diam vel, aliquam felis. Nulla facilisi. Nulla finibus sed tortor non imperdiet. Aliquam a risus malesuada, sodales justo sit amet, posuere mi. Quisque varius porta ex. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
                        <p class="pararealismo">Maecenas sem lorem, luctus sit amet finibus ut, lobortis eu orci. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Mauris lobortis at ipsum quis sodales.Ut sapien eros, tincidunt et porttitor vitae, fringilla vulputate justo. Sed porttitor libero vitae ex feugiat, at imperdiet lacus convallis. Aenean vel commodo neque, eget mollis eros. Mauris posuere tincidunt sapien at suscipit. Nam tempus ornare arcu in accumsan. Sed vitae fringilla orci. Aliquam neque erat, lacinia eu dictum et, molestie in tortor. Suspendisse posuere at dolor at mattis. Quisque nec ornare metus.Nullam porta auctor sem, at pharetra augue fermentum id. Fusce nec orci eget nisl laoreet elementum et eu massa.</p>
                    </aside>

                </div>
                <div class="row">
                    <h2 class="parameio titulo centro ajustamento">Principais obras realistas</h2>
                    <div class="realismodivisor"></div>
                    <section class="col-xs-12 col-md-4 col-lg-4">
                        <h4 class="h4realismobr line">O Cortiço</h4>
                        <img src="img/realismo/cortico.jpg" class="col-xs-12 col-lg-6 arrumarpimg" alt="Capa de O Cortiço de Aluísiao de Azevedo.">
                        <p class="pararealismo">Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio cumque nihil impedit quo minus id quod maxime placeat facere possimus.Nunc sodales ornare eros. Praesent feugiat semper sollicitudin. Morbi egestas lorem leo, vitae feugiat nisl aliquam nec. Morbi dictum nisl neque. Donec pulvinar ullamcorper bibendum. Cras consequat lorem urna, id placerat risus feugiat at. Fusce vel libero non eros iaculis ultrices nec vitae nulla. Aliquam sollicitudin sem ac mi molestie, id pulvinar dui facilisis.</p>
                    </section>
                    <section class="col-xs-12 col-md-4 col-lg-4">
                        <h4 class="h4realismobr line">O Atheneu</h4>
                        <img src="img/realismo/ateneu.jpg" class="col-xs-12 col-md-4 col-lg-6 arrumarpimg" alt="Capa de O Atheneu de Raul Pompeia.">
                        <p class="pararealismo">Lorem leo morbi egestas, vitae feugiat nisl aliquam nec. Morbi dictum nisl neque. Donec pulvinar ullamcorper bibendum. Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio cumque nihil impedit quo minus id quod maxime placeat facere possimus.Cras consequat lorem urna, id placerat risus feugiat at. Fusce vel libero non eros iaculis ultrices nec vitae nulla. Aliquam sollicitudin sem ac mi molestie, id pulvinar dui facilisis.Nunc sodales ornare eros. Praesent feugiat semper sollicitudin. </p>
                    </section>
                    <section class="col-xs-12 col-md-4 col-lg-4">
                        <h4 class="h4realismobr line">Inocência</h4>
                        <img src="img/realismo/inocencia.jpg" class="col-xs-12 col-lg-6 arrumarpimg" alt="Capa de Inocência de  Visconde de Taunay.">
                        <p class="pararealismo">Vitae feugiat nisl aliquam nec, lorem leo morbi egestas. Morbi dictum nisl neque. Donec pulvinar ullamcorper bibendum. Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio cumque nihil impedit quo minus id quod maxime placeat facere possimus.Cras consequat lorem urna, id placerat risus feugiat at. Fusce vel libero non eros iaculis ultrices nec vitae nulla.Id pulvinar dui facilisis.Nunc sodales ornare eros. Praesent feugiat semper sollicitudin. Aliquam sollicitudin sem ac mi molestie.</p>
                    </section>
                </div>
            </div>
        </main>

        <!-- Rodapé
    –––––––––––––––––––––––––––––––––––––––––––––––––– -->
        <?php include 'includes/estrutura/footer.php'; ?>

</body>

</html>
